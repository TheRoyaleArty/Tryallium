...
public class MachinetryalliumModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "machinetryallium_mapvars";
		public double timerstickrouge = 300;
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void readFromNBT(NBTTagCompound nbt) {
			timerstickrouge = nbt.getDouble("timerstickrouge");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setDouble("timerstickrouge", timerstickrouge);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				MachinetryalliumMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				MachinetryalliumMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "machinetryallium_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				MachinetryalliumMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				MachinetryalliumMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					MachinetryalliumMod.PACKET_HANDLER.sendToAll(message);
				else
					MachinetryalliumMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
